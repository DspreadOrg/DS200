 ...
 *
 **************************************/
extern unsigned char __scratch2_start;
static asr_prop_area *__system_property_area__;
...
{
  __system_property_area__ = (asr_prop_area *)((uintptr_t)&__scratch2_start + ASR_PROP_AREA_OFFSET);
  asr_prop_area *pa = __system_property_area__;
  ...
  if(!force_init) {
    if(pa->magic == ASR_PROP_AREA_MAGIC) {
      ...
      memset(m_anchor, 0, ((unsigned)pa + ASR_PA_SIZE - (int)m_anchor));
