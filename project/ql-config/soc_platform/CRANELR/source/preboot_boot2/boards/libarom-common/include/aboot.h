...
#define FLASHER_PTABLE_ENTRY_SIZE   64
#else
#define FLASHER_PTABLE_ENTRY_SIZE   32
#endif
/*---------------------------------------------------------------------------*/
/*
...
typedef struct {
  flasher_cmd_t cmd;
  char partition[FLASHER_PTABLE_ENTRY_SIZE];
  void *data;
  size_t size;
} flasher_message_t;
...
void aboot_register_boot_callback(reboot_cmd_t cb);
void aboot_register_go_callback(reboot_cmd_t cb);
void aboot_reboot_delay_set(unsigned ms);
unsigned aboot_reboot_delay_get(void);
