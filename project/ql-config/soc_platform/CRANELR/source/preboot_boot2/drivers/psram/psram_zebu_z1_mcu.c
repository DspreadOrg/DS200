...
psram_init_zebu_z1_mcu(void)
{
    ...
    LOG_INFO("psram_init_zebu_z1_mcu: Z2\n");
    LOG_INFO("psram_init  - -  cranew new psram init for MCU !!! --EMULATOR \n");
...
#else
        /* 208M huwen set */
        REG32(0x410000b0) &= ~(0x1f << 16);
        REG32(0x410000b0) |= (1 << 23);
        read_data = REG32(0x410000b0);
        /* wait for freg change over */
        while (REG32(0x410000b0) & (1 << 23)) {}
#endif
    }
