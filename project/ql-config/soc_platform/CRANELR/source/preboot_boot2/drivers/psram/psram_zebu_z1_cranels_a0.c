...
psram_init_zebu_z1_cranels_a0(void)
{
    ...
    LOG_INFO("Psram init for craneLS A0!!\n");
    ...
    REG32(0xd42828b0) &= ~(0x7 << 18);
    if (1) {
        REG32(0xd42828b0) &= ~(0x1 << 8);
        REG32(0xd42828b0) |= (1 << 23) | (0x0 << 18) | (1 << 8); /*config for clk */
    }
    else {
...
/*SEQ_TABLE*/ REG32(PSRAM_BASE + 0x8000) |= ((0x1 << 24) | 0x1);
    ...
    psram_read_mr(mr2_addr, 0);
    psram_read_mr(mr3_addr, 0);

    psram_read_mr(mr0_addr, 1);
    psram_read_mr(mr1_addr, 1);
    psram_read_mr(mr2_addr, 1);
    psram_read_mr(mr3_addr, 1);
}
