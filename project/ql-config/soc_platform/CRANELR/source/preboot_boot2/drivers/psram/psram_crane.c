...
/*
 * WB4MB: enable fast refresh rate, see psram_wb_32Mb_init_silicon.c
 * WB8MB: enable fast refresh rate
 * WB_16M and WB_32M: not enable fast refresh rate yet
 */
 ...
 */
static bool is_chip_support_auto_detect(void)
{
    if (is_cranelr()) {
        return true;
    }
...
static unsigned psram_fuse(void)
{
    ...
    unsigned version_info = efuse_get_psram_version();
    ...
    switch (version_info) {
        ...
        case WB_32M:
            ...
            break;
        case AP_250MHZ_16M:
            memcpy(psram_str, "AP_250MHZ_16M", PROP_VALUE_MAX);
            break;
        case WB_250MHZ_16M:
            memcpy(psram_str, "WB_250MHZ_16M", PROP_VALUE_MAX);
            break;
        default:
