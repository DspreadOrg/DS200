...
mpu_region_config(unsigned region_num, unsigned base, unsigned size, unsigned acc_ctrl)
{
  ...
  if(region_num < MAX_PRIORITY_USER_REGION_NUM) {
  ...
  return 0;
error:
  printf("mpu_region_config failed\n");
  return -1;
}
/*---------------------------------------------------------------------------*/
int
...
/*---------------------------------------------------------------------------*/
/*
 * transfer control use rfe, the transfer parameter in btcm, need btcm to be stronly-ordered firstly.
 * TEX C  B  stronly-ordered
 * 000 0  0
 */
int
set_mpu_btcm(uintptr_t base, size_t size)
{
  /* Grant read, write and excution privilege to the code area */
  unsigned int acc_ctrl = MPU_REGION_ACCESS_CTRL_AP(0x3);         /* read, write */
  //int index = arom_mpu_get_next();
  int index = BTCM_CODE_REGION_NUM;

  return mpu_config(index, base, size, acc_ctrl);
}
/*---------------------------------------------------------------------------*/
