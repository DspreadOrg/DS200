...
/*
 * QSPI LUT config struct, all field -1 means end of table
 */
typedef struct {
...
typedef union {
    struct {
        unsigned reserved : 8;      /* bit 7:0 */
        unsigned trbfl    : 8;      /* bit 15:8 */
        unsigned trctr    : 16;     /* Bit 31:16 */
    } s;
    unsigned value;
} qspi_tbsr_t;

int  qspi_init(int clock_freq, unsigned offset_addr);
void qspi_set_flash_offset_addr(unsigned offset_addr);
unsigned qspi_get_flash_offset_addr(void);
...
/* The data should be 8 bytes aligned */
int  qspi_write_dma(unsigned seqid, unsigned addr, const void *data, size_t size);
