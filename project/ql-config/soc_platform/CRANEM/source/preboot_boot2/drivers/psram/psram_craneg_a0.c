...
static bool is_wb(unsigned version)
{
    return ((version == WB_8M8M || version == WB_250MHZ_8M8M || version == WB_8M || version == WB_250MHZ_8M)) ? true : false;
}

static bool is_ap_8M8M(unsigned version)
{
    return (version == AP_8M8M) ? true : false;
}

static bool is_uhs(unsigned version)
{
    return (version == AP_UHS_8M8M) ? true : false;
}
...
static unsigned psram_fuse(void)
{
    ...
    if (version_info == 0) {
        version_info = WB_8M8M;
        LOG_PRINT("No fuse bit , fake it to WB_8M8M instead of AP_16M\n");
    }
#endif
    LOG_INFO("########################\n");
    ...
    switch (version_info) {
        case AP_16M:
            ...
            break;
        case WB_16M:
            memcpy(psram_str, "WB 16MB", PROP_VALUE_MAX);
            break;
        case AP_8M8M:
        ...
        case AP_8M:
            ...
            break;
        case WB_8M:
            memcpy(psram_str, "WB 8MB", PROP_VALUE_MAX);
            break;
        case WB_250MHZ_8M:
...
void psram_init_craneg_a0(void)
{
    unsigned version;
    unsigned mr0_addr = 0x0;
    unsigned mr1_addr = 0x1;

    LOG_PRINT("[PSRAM] psram init CraneG .\n");
