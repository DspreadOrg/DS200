...
pmic_setup(void)
{
  ...
  pmmic_LDO_config_for_cranew_mcu_and_pm813s();
#endif
...
pmic_set_buck1_vol(unsigned int chip_id)
{
  ...
  if(PM816 == PMIC_PID(pmic_id)) {
    ...
    /* default is 0x20, represents 0.8v, Cr5 need to set buck1 0.9v,so set 0.9v here */
    val = 0x2A;
 ...
 */
void
pmmic_LDO_config_for_cranew_mcu_and_pm813s(void)
{
  unsigned char val;
