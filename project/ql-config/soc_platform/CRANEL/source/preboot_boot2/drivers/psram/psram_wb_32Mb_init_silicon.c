...
void wb32Mb_psram_fast_refresh(void)
{
    unsigned read_data = 0;
    
    LOG_INFO("wb32Mb_psram_fast_refresh\r\n");
    
    REG32(PSRAM_BASE + 0x8024) &= ~(0xf<<12);
    REG32(PSRAM_BASE + 0x9024) &= ~(0xf<<12);
    
    REG32(PSRAM_BASE + 0x8024) |= (0x4<<12);
    REG32(PSRAM_BASE + 0x9024) |= (0x5<<12);

    // write CR0 to take effect:
...
#else   // ~FIX_LATENCY
#ifdef DIS_PSC_CACHE
    REG32(PSRAM_BASE + 0x8024) = (0x078f & WBD_MRB_MASK) | (WBD_DRIVE << WBD_MRB_OFST);
    REG32(PSRAM_BASE + 0x9024) = (0x178f & WBD_MRB_MASK) | (WBD_DRIVE << WBD_MRB_OFST);
#else   // ~DIS_PSC_CACHE
    REG32(PSRAM_BASE + 0x8024) = (0x048f & WBD_MRB_MASK) | (WBD_DRIVE << WBD_MRB_OFST);
    REG32(PSRAM_BASE + 0x9024) = (0x148f & WBD_MRB_MASK) | (WBD_DRIVE << WBD_MRB_OFST);
#endif  // ~DIS_PSC_CACHE end
#endif  // ~FIX_LATENCY end
#endif  // WBD_32Mb end
