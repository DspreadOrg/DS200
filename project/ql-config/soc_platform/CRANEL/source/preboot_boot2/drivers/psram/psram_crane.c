...
static bool is_cranels(void);
static bool is_cranew(void);
...
static bool is_cranew(void)
{
    ...
    return (CHIP_ID_CRANEW == chip_id) ? true : false;
}
...
static unsigned psram_fuse(void)
{
    ...
    unsigned version_info = efuse_psram();
    ...
    if (is_crane()) {
        ...
        version_info &= 0x7;
             ...
             is_cranels()) {
        /*craneL A0:
         * MSB: Bank0 Bit[93]: AP[77]
         * LSB: Bank3 Bit[171~168]
         */
        unsigned int apcfg3 = read32(GEU_FUSE_VAL_APCFG3);
        unsigned int cranel_psramtype_msb = (apcfg3 & (1 << 13)) ? 1 : 0;
        version_info = (cranel_psramtype_msb << 4) | (version_info & 0xF);
    }
...
#define WB_BIT_REVERSE

#ifdef WB_BIT_REVERSE
 int psram_write_testmode(unsigned seq_addr)
{
    unsigned read_data = 0;
 ...
 int winbond_psram_fast_refresh_entry(void)
{
 ...
 static int winbond_psram_fast_refresh_entry_test(void)
 {
    ...
    reg = PSRAM_REG32(PSRAM_BASE + 0x4010);
	reg |= (1<<12);
	PSRAM_REG32(PSRAM_BASE + 0x4010) = reg;
...
#ifdef WB_BIT_REVERSE
    {
        LOG_INFO("wb psram, fast refreshs\n");
        for (i = 0; i < 1000; i++) {}
