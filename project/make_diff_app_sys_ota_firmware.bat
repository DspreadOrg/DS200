...
if exist %CURDIR%\ql-cross-tool\FBFMake_CF\new\customer_app.bin (
	...
	echo 请先将customer_app.bin拷贝到%CURDIR%\ql-cross-tool\FBFMake_CF\new
	echo Copy customer_app.bin to %CURDIR%\ql-cross-tool\FBFMake_CF\new
    goto make_ota_end
)

if exist %CURDIR%\ql-cross-tool\FBFMake_CF\old\customer_app.bin (
	echo off
)else (
	echo 请先将customer_app.bin拷贝到%CURDIR%\ql-cross-tool\FBFMake_CF\old
	echo Copy customer_app.bin to %CURDIR%\ql-cross-tool\FBFMake_CF\old
    goto make_ota_end
)
pushd ql-cross-tool\FBFMake_CF\
call fbfmake_diff_app_system.bat
popd

if exist %CURDIR%\ql-cross-tool\FBFMake_CF\app_sys_fbf_dfota.bin (
...
set buildver_name=APP_SYS_DFOTA.bin


move /y %ota_fw_path%\app_sys_fbf_dfota.bin %buildver_name%
echo OTA固件制作成功
